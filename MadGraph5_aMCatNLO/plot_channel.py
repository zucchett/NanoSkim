...
c_min = -1.0
c_max =  1.0
m_min = {"Z":  70.0, "H": 124.95, "JPsi": 3.095, "PsiPrime": 3.684}
m_max = {"Z": 110.0, "H": 125.05, "JPsi": 3.099, "PsiPrime": 3.688}
...
if (S not in S_list):
    ...
    sys.exit()
if (M not in M_list):
    sys.stdout.write("Wrong meson product inserted")
    sys.exit()
...
################################################################################
COS_THETA_STAR_STR   = "cos(#theta_{" + M_root[M] + "}*)"
COS_THETA_1_MU_P_STR = "cos(#theta_{" + M_root[M] + ",#mu^{#scale[1.5]{+}}})"
...
h3 = TH1D("h3", "#bf{" + PROC_ID + "}: " + COS_THETA_1_MU_M_STR + " distribution", nbins, c_min, c_max);
h4 = TH1D("h4", "#bf{" + PROC_ID + "}: " + M_MUMU_STR           + " distribution", nbins, m_min[M], m_max[M]);
h5 = TH1D("h5", "#bf{" + PROC_ID + "}: " + M_MUMUGAMMA_STR      + " distribution", nbins, m_min[S], m_max[S]);
...
if not os.path.isdir(SAVE_PATH_SMP):
    os.system("mkdir " + SAVE_PATH_SMP)

# plot cosTheta*
c1 = TCanvas("c1", "c1", 800, 600);
h1.GetXaxis().SetTitle(COS_THETA_STAR_STR);
...
h1.Draw("HIST");
c1.SaveAs(SAVE_PATH_SMP + PROC_ID + "_cosThetaStar.png")
c1.SaveAs(SAVE_PATH_SMP + PROC_ID + "_cosThetaStar.pdf")
c1.Draw()

# plot cosTheta1 (jpsi-mu-)
c2 = TCanvas("c2", "c2", 800, 600);
h2.GetXaxis().SetTitle(COS_THETA_1_MU_P_STR)
...
h2.Draw("HIST");
c2.SaveAs(SAVE_PATH_SMP + PROC_ID + "_cosTheta1_mu_p.png")
c2.SaveAs(SAVE_PATH_SMP + PROC_ID + "_cosTheta1_mu_p.pdf")
c2.Draw()

# plot cosTheta1 (jpsi-mu+)
c3 = TCanvas("c3", "c3", 800, 600);
h3.GetXaxis().SetTitle(COS_THETA_1_MU_M_STR)
...
h3.Draw("HIST");
c3.SaveAs(SAVE_PATH_SMP + PROC_ID + "_cosTheta1_mu_m.png")
c3.SaveAs(SAVE_PATH_SMP + PROC_ID + "_cosTheta1_mu_m.pdf")
c3.Draw()

# plot m_{mumu}
c4 = TCanvas("c4", "c4", 800, 600);
h4.GetXaxis().SetTitle(M_MUMU_STR + " [GeV]")
...
h4.Draw("HIST");
c4.SaveAs(SAVE_PATH_SMP + PROC_ID + "_m_mumu.png")
c4.SaveAs(SAVE_PATH_SMP + PROC_ID + "_m_mumu.pdf")
c4.Draw()

# plot m_{mumugamma}
c5 = TCanvas("c5", "c5", 800, 600);
h5.GetXaxis().SetTitle(M_MUMUGAMMA_STR + " [GeV]")
...
h5.Draw("HIST");
c5.SaveAs(SAVE_PATH_SMP + PROC_ID + "_m_mumugamma.png")
c5.SaveAs(SAVE_PATH_SMP + PROC_ID + "_m_mumugamma.pdf")
c5.Draw()
################################################################################
