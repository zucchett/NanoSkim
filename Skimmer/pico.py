...
parser.add_option('-i', '--input', action='store', type='string', dest='origin', default='/lustre/cmsdata/zucchett/SameSign/v2/')
parser.add_option('-o', '--output', action='store', type='string', dest='target', default='/lustre/cmsdata/zucchett/SameSign/Gruppo2/')
...
skimString = "((iSkim==1 && isSingleMuIsoTrigger && Muon_charge[0]*Muon_charge[1]>0))" # || (iSkim==2 && isSingleMuIsoTrigger && Muon_charge[0]*Electron_charge[0]>0) || (iSkim==6 && isSingleEleIsoTrigger && Electron_charge[0]*Electron_charge[1]>0))"
#skimString = "( (iSkim==1 && isSingleMuIsoTrigger && Muon_charge[0]*Muon_charge[1]>0) || (iSkim==2 && isSingleMuIsoTrigger && Muon_charge[0]*Electron_charge[0]>0) || (iSkim==3 && isSingleMuIsoTrigger) || (iSkim==4 && isSingleMuIsoTrigger) )"
#skimString = "isSingleMuonPhotonTrigger && nMuon >= 2 && nPhoton >= 1 && nCleanMuon == 0 && nCleanPhoton == 0 && nCleanElectron == 0 && Muon_pt[iMuon1] > 18. && Muon_pt[iMuon2] > 5. && Muon_mediumId[iMuon1] && Muon_mediumId[iMuon2] && minMuonIso < 0.05 && maxMuonIso < 0.15 && Photon_pt[iPhoton] > 32. && Photon_mvaID_WP80[iPhoton] && Photon_pfRelIso03_all[iPhoton] < 0.05 && (JPsi_mass > 3.0 && JPsi_mass < 3.2) && H_dPhi > 2.0 && abs(Muon_dxy[iMuon1]-Muon_dxy[iMuon2])<0.02 && abs(Muon_dz[iMuon1]-Muon_dz[iMuon2])<0.02"
#skimString = "isJPsiTrigger && nMuon >= 2 && nPhoton >= 1 && nCleanMuon == 0 && nCleanPhoton == 0 && nCleanElectron == 0 && Muon_pt[iMuon1] > 5. && Muon_pt[iMuon2] > 5. && Muon_mediumId[iMuon1] && Muon_mediumId[iMuon2] && minMuonIso < 0.05 && maxMuonIso < 0.15 && JPsi_pt > 27. && Photon_pt[iPhoton] > 15. && Photon_mvaID_WP80[iPhoton] && Photon_pfRelIso03_all[iPhoton] < 0.05 && (JPsi_mass > 3.0 && JPsi_mass < 3.2) && H_dPhi > 2.0 && abs(cosThetaStar) < 0.6 && nCleanJet <= 2 && abs(Muon_dxy[iMuon1]-Muon_dxy[iMuon2])<0.010 && abs(Muon_dz[iMuon1]-Muon_dz[iMuon2])<0.02"
